...
DEFAULT_COOKIE_SAVELIST = ["user_opted_remember_device_preference", "remember_device"]
...
def fail_response(response, expected_redirect, expected_text):
    if os.getenv("DEBUG"):
        ...
        if expected_redirect:
            error_msg = (
                f"You wanted {expected_redirect}, but got " f"{response.url} for a url."
            )
            if check_fail_text(response.text):
                error_msg += f" Found the following fail msg(s): " + " | ".join(
                    check_fail_text(response.text)
                )
...
def authenticity_token(response, index=0):
    ...
    if not token:
        ...
        if os.getenv("DEBUG"):
            ...
            logging.error(f"Failed to find authenticity token in {response.url}")
        raise locust.exception.RescheduleTask
    ...
    * a phone number between +1 (415) 555-0001 and +1 (415) 555-1000.
    ...
    if used_nums != None:
        while user_num in used_nums:
            ...
            logging.debug(f"{LOG_NAME}: User #{user_num} ready for service.")
...
def load_fixture(filename, path="./load_testing"):
    ...
    try:
        ...
        try:
            url = (
                "https://github.com/18F/identity-loadtest/raw/main/load_testing/"
                + filename
            )
            r = requests.get(url)
            fixture = r.content
